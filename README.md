...
与原版 Guise 1.1.2 相比，面向目标应用的独立伪装/隐私配置由 **35 项增加到 38 项**，净新增 **3 项**：显示大小（DPI）、时区和应用列表可见性。这里不把主题、预测性返回、更新检测等管理端设置计入伪装项目。
...
- GitHub Actions 会为 Guise 的正式版和 prerelease 生成更新清单；当前仅发布预览版，因此应用内更新检测也会收到最新预发布版本。`guise-test-*` 独立 Tag 仍不会进入 Guise 更新清单。
